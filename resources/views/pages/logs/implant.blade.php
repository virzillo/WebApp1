...
<div class="card card-custom">
    ...
    <div class="card-header flex-wrap border-0 pt-6 pb-0">
        <div class="card-title">
            <h3 class="card-label">Visualizzazioni imipianti
    ...
    <div class="card-body">
        ...
        <table class="table table-bordered table-vertical-center" id="kt_datatable">
            ...
            <tbody>
                @foreach ($implantviews as $implantview)
                <tr>
                    <td>{{ $implantview->id }}</td>
                    <td><a href="{{route('visualizza.impianto',$implantview->slug)}}" target="_new">{{ $implantview->implantView->titolo }}</a></td>
                    <td class="sorting_1 dtr-control">
                        {{ $implantview->slug }}
                    </td>

                    <td>{{ $implantview->url }}</td>
                    <td>{{ $implantview->user_id }}</td>
                    <td>{{ $implantview->ip }}</td>
                    <td>{{ $implantview->created_at->format('d,m,Y') }}</td>
